...
class dBug {
	...
	function varIsArray($var) {
		...
		$c=count($var);
	$this->makeTableHeader("array","array[".$c."]");
		if(is_array($var)) {
			foreach($var as $key=>$value) {
				...
				if(in_array(gettype($value),$this->arrType))
					...
					$value=(trim($value)=="") ? "[empty string2]" : $value;
					echo $value;
				}
	...
	function varIsObject($var) {
		...
		$ctype=get_class($var);
		array_push($this->arrHistory, $var_ser);
		$this->makeTableHeader("object","object[".$ctype."]");
		
		if(is_object($var)) {
			$arrObjVars=get_object_vars($var);
			foreach($arrObjVars as $key=>$value) {

				$value=(!is_object($value) && !is_array($value) && trim($value)=="") ?($value===null)?"[NULL]":"[empty string3]" : $value;
	...
	function varIsDBResource($var,$db="mysql") {
		...
		for($i=0;$i<$numrows;$i++) {
			...
			for($k=0;$k<$numfields;$k++) {
				...
				$fieldrow=($fieldrow=="") ? "[empty string4]" : $fieldrow;
				echo "<td>".$fieldrow."</td>\n";
			}
			echo "</tr>\n";
		}
