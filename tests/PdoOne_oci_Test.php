...
class PdoOne_oci_Test extends TestCase
{
    /** @var PdoOne */
    protected $pdoOne;
        ...
        if(!$this->pdoOne->objectExist('TABLELOG')) {
            self::assertEquals(true,
                $this->pdoOne->createTable('TABLELOG'
                    , ['id' => 'int GENERATED ALWAYS AS IDENTITY not null', 'name' => 'varchar2(45)', 'description' => 'varchar2(45)'],
                    ['id' => 'PRIMARY KEY']));
        }
    ...
    */
    ...
    public function test_dep()
    {
        ...
        for($i=0;$i<2;$i++) {
            foreach ($tables as $table) {
                var_dump($table);
                try {
                    $this->pdoOne->dropTable($table);
                } catch (Exception $ex) {
                    var_dump('unable to delete table ' . $table);
                }
            }
        }
        // create two tables
    ...
    public function test_open()
    {
        ...
        try {
            ...
            self::assertEquals(true, $r, 'Drop failed (expected');
        } catch (Exception $e) {
            $r = false;
        ...
        // we add some values
        $this->pdoOne->set(['ID_CATEGORY' => 123, 'CATNAME' => 'cheap'])->from('PRODUCT_CATEGORY')->insert(null,null,PdoOne::NULL);
        ...
        self::assertEquals(123, $this->pdoOne->from('PRODUCT_CATEGORY')->max('ID_CATEGORY'),
            'max must value 123');
        $avg=$this->pdoOne->from('PRODUCT_CATEGORY')->avg('ID_CATEGORY');
        self::assertEquals(true , abs($avg-27.4)<0.001,
            'avg must value 27.4');
        self::assertEquals([
            ['ID_CATEGORY' => 2],
            ['ID_CATEGORY' => 3],
    ...
    public function test_quota()
    {
        ...
        $this->assertEquals('"hello" =value',$this->pdoOne->addDelimiter('hello =value'));
        $this->assertEquals('2019-01-01',PdoOne::dateConvert('01/01/2019','human','sql'));
    ...
    {
        self::assertEquals([0 => [1 => 1]], $this->pdoOne->runRawQuery('select 1 from dual', null, true));
    }
