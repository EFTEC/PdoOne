...
class PdoOne_mysql_Test extends TestCase
{
    ...
    function test_genCode()
    {
        ...
        $this->assertContains("array \$result=array(['id'=>0])",
    ...
    public function test_open()
    {
        ...
        $this->pdoOne->insert('product_category', ['id_category' => 'i', 'catname' => 's'],
        ...
        $count = $this->pdoOne->select('select catname from product_category')
            ->where('id_category=:idcat', ['idcat' => 4])->firstScalar();
        $this->assertEquals('cheap4', $count, 'insert must value cheap4');
        $this->assertEquals(137, $this->pdoOne->sum('id_category')->from('product_category')->firstScalar(),
            'sum must value 137');
        $this->assertEquals(2, $this->pdoOne->min('id_category')->from('product_category')->firstScalar(),
            'min must value 2');
        $this->assertEquals(123, $this->pdoOne->max('id_category')->from('product_category')->firstScalar(),
            'max must value 123');
        $this->assertEquals(27.4, $this->pdoOne->avg('id_category')->from('product_category')->firstScalar(),
            'avg must value 27.4');
        $this->assertEquals([
                ['id_category' => 2],
                ['id_category' => 3],
    ...
    public function test_emptyargs()
    {
        ...
        $this->assertEquals(['id_category' => 1, 'catname' => 'expensive'],
            $this->pdoOne->select('select * from product_category where id_category=1')->first());
        $this->assertEquals(['id_category' => 2, 'catname' => 'cheap1'],
    ...
    public function test_sequence()
    {
        ...
        $this->assertLessThan(3639088446091303982, $this->pdoOne->getSequence(true),
            'sequence must be greater than 3639088446091303982');
    }

    public function test_sequence2()
    {
        $this->assertLessThan(3639088446091303982, $this->pdoOne->getSequencePHP(false),
            'sequence must be greater than 3639088446091303982');
    ...
    public function test_sqlGen()
    {
        ...
        $this->assertEquals('select 1, 2 from DUAL',
        ...
        $this->assertEquals('select 1, 2 from DUAL where field=:field',
            $this->pdoOne->select(['1', '2'])->from('DUAL')->where('field=:field', [20])->sqlGen(true));
    ...
    {
        $this->assertEquals(true, $this->pdoOne->runQuery($this->pdoOne->prepare('select 1 from dual')));
        $this->assertEquals([1 => 1], $this->pdoOne->select('1')->from('dual')->first(), 'it must runs');
    }
    ...
    public function test_setEncryptionINTEGER()
    {
        ...
        $this->assertTrue(strlen($value) > 3, 'Encrypted');
        $this->assertEquals($original, $this->pdoOne->decrypt($value), 'decrypt correct');
        // 1
        $original = 1;
        $value = $this->pdoOne->encrypt($original);
        $this->assertTrue(strlen($value) > 3, 'Encrypted');
    ...
    public function test_setEncryptionSIMPLE()
    {
        ...
        $original = 'abc';
        $value = $this->pdoOne->encrypt($original);
        $this->assertEquals('wrzS', $value, 'encrypt with problems');
        $this->assertEquals($original, $this->pdoOne->decrypt($value), 'decrypt with problems');
        ...
        $original = 1222;
        $value = $this->pdoOne->encrypt($original);
        $this->assertEquals('koyhkw==', $value, 'encrypt with problems');
        $this->assertEquals($original, $this->pdoOne->decrypt($value), 'decrypt correct');
        // 0
        $original = 0;
        $value = $this->pdoOne->encrypt($original);
        $this->assertEquals('kQ==', $value, 'encrypt with problems');
        $this->assertEquals($original, $this->pdoOne->decrypt($value), 'decrypt correct');
    }
}
