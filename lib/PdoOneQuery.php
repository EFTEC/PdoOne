...
class PdoOneQuery {
    ...
    public function toMeta($sql = null, $args = [])
    {
        ...
        if ($sql === null) {
            ...
            /** @var PDOStatement $stmt */
            $stmt = $this->parent->runRawQuery($sql, $args, false);
        }
     ...
     *
     * @param string $needle
     * @param null|array $recursiveArray If null then it uses the recursive array specified by $this->parent->>recursive();
     *
     * @return bool
     */
    ...
    )
    {
        ...
        if ($this->parent->useCache === true) {
            $this->parent->invalidateCache('', $this->parent->cacheFamily);
        }
        if ($tableDef !== null) {
        ...
        $this->parent->beginTry();
        $this->parent->runRawQuery($sql, $param);
    ...
    )
    {
        ...
        $this->parent->beginTry();
        $stmt = $this->parent->runRawQuery($sql, $param, false);
        $this->builderReset(true);
        if ($this->endtry() === false) {
            return false;
        }
    ...
    )
    {
        ...
        if ($useCache !== false && $returnArray) {
            ...
            $result = $this->parent->cacheService->getCache($this->uid, $this->parent->cacheFamily);
            if ($result !== false) {
        ...
        if ($returnArray && $stmt instanceof PDOStatement) {
            ...
            if ($extraIdCache === 'rungen' && $this->uid) {
                // we store the information of the cache.
                $this->parent->setCache($this->uid, $this->parent->cacheFamily, $result, $useCache);
            }
            $this->builderReset();
            if ($uid !== false) {
                $this->parent->internalCache[$uid] = $result;
